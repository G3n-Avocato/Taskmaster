...
bool    parsing_name(t_process_para* para, char* val) {
    ...
    if (!double_name_parsing(para, val))
        return false ;
...
bool    double_name_parsing(t_process_para* para, char* val) {

    for (unsigned int i = 0; i < para->count; i++) {
        if (!strcmp(para->config[i].name, val)) {
            fprintf(stderr, "Error parser config : Duplicate group name \'%s\' in configuration file\n", val);
            return false;
        }
    }
    return true ;
