...
bool    logger_parser_name_file(char **argv, int argc) {
    ...
    if (strcmp(tmp, ".yaml") != 0) {
        if (strcmp(tmp, ".yml") != 0) {
            logger(ERRO, "Wrong type for config file");
            return false ;
        }
    }

    if (access(argv[1], F_OK) == -1) {
        logger(ERRO, "Config file does not exist");
        return false ;
    }
    if (access(argv[1], R_OK) == -1) {
        logger(ERRO, "Config file not accessible");
        return false ;
    }
...
bool    logger_parser_file_yaml(char *file, t_process_para* para) {
    ...
    if (!fd) {
        char    buffer[100];
        snprintf(buffer, sizeof(buffer), "Error config file : %s", strerror(errno));
        logger(CRIT, buffer);
        return false ;
    }
    ...
    while (!done) {
        if (!logger_syntax_error_file_config(&parser, &event, fd)) {
            if (last_key)
                free(last_key);
            return false ;
        }
        switch (event.type) {
            case YAML_SCALAR_EVENT:
                ...
                if (state == ST_INIT) {
                    if (!strcmp(val, "programs"))
                        ...
                        logger(ERRO, "Error config file : Expected 'programs' key");
                        free(val);
                        free_lib_yaml(&parser, &event, fd);
                        return false ;
                    }
                    ...
                    if (!logger_parsing_name(para, val)) {
                    ...
                    if (!last_key) {
                        ...
                        if (!logger_parser_list_options_config(last_key, val, cfg)) {
                            free_var_yaml(&val, &last_key);
                            free_lib_yaml(&parser, &event, fd);
                            return false ; 
                        }
            ...
            case YAML_SEQUENCE_START_EVENT:
                if (last_key && !strcmp(last_key, "exitcodes")) {
                    ...
                    if (!logger_parser_exitcodes(&parser, &event, cfg)) {
                        free(last_key);
                        free_lib_yaml(&parser, &event, fd);
                        return false;
                    }
            ...
            case YAML_MAPPING_START_EVENT:
                if (last_key && !strcmp(last_key, "env")) {
                    ...
                    if (!logger_parser_env(&parser, &event, cfg)) {
                        free(last_key);
                        free_lib_yaml(&parser, &event, fd);
                        return false;
                    }
    ...
    if (!logger_parser_option_config_requis(para))
        return false ;
