...
bool    main_loop(t_superMap** superMap, t_process_para* para, t_ctrl_cmds* ctrl) {
    ...
    while (running) {
        ...
        if (now.tv_sec - last_check.tv_sec >= 2) {
...
bool    logRotate_loop(t_superMap** superMap) {
    (void)superMap;
    for (int i = 0; i < g_processCount; i++) {
        monitor_log_file((*superMap)[i].proc.exec->stdout, (*superMap)[i].proc.processus, true);
        monitor_log_file((*superMap)[i].proc.exec->stderr, (*superMap)[i].proc.processus, false);
    }
    return true ;
}
